...
1. 缓存空值，查不到数据时，将key的值设置为null，但是要记得设置过期时间(不能从根本上解决问题)
2. 布隆过滤器：如果判定存在，则可能存在，如果不存在，则一定不存在，存在一定误判率
3. 接口层增加校验，不合法的参数直接返回，比如id < 0,  分页参数不能过大(不仅可能造成慢查询，还可能会OOM，GC)



#### 2. 缓存雪崩

![image-20200914000401582](redis.assets/image-20200914000401582.png)

缓存设置了相同的过期时间，导致在某一时刻大量缓存过期，集中请求数据库导致DB压力过大

##### 解决方案

1. 在原有的失效时间的基础上加上一个随机值，比如1-5min
2. 增加redis定时器执行频率
...
1. 使用分布式互斥锁，缓存拿不到数据时，先获取锁，获取成功才去DB查数据，查询成功则放入缓存
2. 热点key永不过期
